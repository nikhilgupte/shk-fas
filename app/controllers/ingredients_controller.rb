class IngredientsController < ApplicationController
  ...
  def export
    if request.format == Mime::CSV
        ...
        return render :text => @ingredients.collect{|i| ([i.code] + Currency.all.collect{|c| i.latest_price.send(c.name.downcase).round(2)} + [(i.latest_price.user.username rescue ''), i.latest_price.created_at.to_s(:datetime)]).to_csv}.insert(0, (['code'] + Currency.all.collect{|c| c.name} + %w(user updated_on)).to_csv).join
    else
